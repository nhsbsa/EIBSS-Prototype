(function (root) {
  ...
  if (!element) return; // a temporary value to cache what we're about to show

  var target = null;
  var tabsList = document.querySelectorAll(".tabs__list")[0];
  var tabs = document.querySelectorAll(".tabs__toggle");
  var panels = document.querySelectorAll(".tabs__panel"); // collect all tabs
  ...
  tabsList.setAttribute("role", "tablist"); // get an array of the panel IDs (from the anchor hash)
  ...
  function show(id) {
    ...
    tabs.forEach(function (el) {
      var match = el.hash === id; // Conditional (ternary) operator

      el.classList[match ? "add" : "remove"]("tabs__toggle--selected");
      el.setAttribute("aria-selected", match);
    });
    panels.forEach(function (el) {
      var match = "#" + el.id === id; // Conditional (ternary) operator
  ...
  window.addEventListener("hashchange", update); // initialise
